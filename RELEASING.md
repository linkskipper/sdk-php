...
3. Commit, then create an ANNOTATED tag in each repo with the SAME tag and push it:
   `git tag -a vX.Y.Z -m "vX.Y.Z" && git push origin vX.Y.Z`
4. Packagist: auto-updates from the new tag via the GitHub webhook set up when the package was
   submitted. The `release` workflow also creates the GitHub Release from this version's
